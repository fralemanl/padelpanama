...
import PlayerRadar from "@/ranking/components/PlayerRadar";
...
const resolveColumnIndex = (row, fallbackIndex, headerNames = []) => {
  const headers = row?.__headers || [];
  const normalizedHeaders = headers.map((header) => normalizeText(header));
  const target = headerNames.map((name) => normalizeText(name));
  const idx = normalizedHeaders.findIndex((header) => target.includes(header));
    ...
    fetchPlayers(gender)
      .then((players) => {
        ...
        const normalized = (players || []).map((row) => {
          ...
          const appIdIndex = resolveColumnIndex(row, COLUMN_INDEX.APP_ID, [
            "app id",
            "appid",
            "id",
          ]);
          const appIdValue = getColumnValue(row, appIdIndex);
  ...
  if (!player) {
    return (
      <div style={{maxWidth: "80rem", margin: "0 auto", padding: "2rem 1rem"}}>
        <Link
          href={`/ranking?gender=${encodeURIComponent(
            gender,
